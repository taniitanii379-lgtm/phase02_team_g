<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <form id="quiz-form" action="{{ route('quizzes-management.store') }}" method="POST">
                @csrf 

                {{-- タイトル --}}
                <div class="mb-4">
                    <label for="title" class="block font-medium text-sm text-gray-700">クイズ名</label>
                    <input type="text" name="title" id="title" class="border-gray-300 rounded-md shadow-sm mt-1 block w-full"
                           value="{{ old('title') }}" placeholder="例：ことわざクイズ" required>
                </div>
                ...
                <div id="questions-wrapper">
                    @php $oldQuestions = old('questions', [['question'=>'','choices'=>['','','',''],'answer'=>0]]); @endphp
                    @foreach ($oldQuestions as $index => $q)
                        <div class="question-block mb-6 border p-4 rounded">
                            ...
                            <input type="text" name="questions[{{ $index }}][question]" class="border-gray-300 rounded-md shadow-sm mt-1 block w-full mb-2"
                                   value="{{ $q['question'] }}" required>

                            <label class="block font-medium text-sm text-gray-700">選択肢（2つ以上）</label>
                            @for ($i = 0; $i < 4; $i++)
                                <input type="text" name="questions[{{ $index }}][choices][]" class="border-gray-300 rounded-md shadow-sm mt-1 block w-full mb-1 choice-input"
                            ...
                            <input type="number" name="questions[{{ $index }}][answer]" class="border-gray-300 rounded-md shadow-sm mt-1 block w-full"
                ...
                <button type="button" id="add-question" class="mb-4 px-4 py-2 bg-blue-600 text-white rounded">問題を追加</button>

                <div class="mt-4">
                    <button type="submit" class="block px-6 py-2 rounded text-white font-semibold bg-blue-600 hover:bg-blue-700 focus:ring-2 focus:ring-blue-400 focus:outline-none transition duration-200">登録する</button>
                </div>
            </form>
        </div>
    </div>
